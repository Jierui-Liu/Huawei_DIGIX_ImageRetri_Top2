...
ce_tri_ratio=2
numofGPU=1
data_dir="/home/LinHonghui/Datasets/HW_ImageRetrieval/train_data"

#=====stage 1 config
stage1_images_per_classes=5
stage1_classes_per_minibatch=6
...
stage2_classes_per_minibatch=5
...
def getlr(epoch,step):
    if step<=stage1_numofstep:
        ...
        lr_based=0.0002
        lr=lr_based*(0.9999503585)**(step-stage1_numofstep)
    return lr
...
class Model(nn.Module):
    def __init__(self):
        ...
        self.backbone_before_attention = nn.Sequential(*model_forzen_part,*model_trainable_part[:1])
        self.backbone_after_attention = nn.Sequential(*model_trainable_part[1:])
        self.self_attention=self_attention(in_channels=1280)
    ...
    def input2embedding(self,x): #必须有，用于提取embedding
        x=self.backbone_before_attention(x)  #3,1280,32,32
