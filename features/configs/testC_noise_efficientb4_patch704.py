...
LastEditTime: 2020-11-04 22:57:30
'''
...
config = dict(
    ...
    #                           
    train_pipeline = dict(
        dataloader = dict(batch_size = 8,num_workers = 12,drop_last = True,pin_memory=True,
                        collate_fn="my_collate_fn"),

        dataset = dict(type="train_dataset",
                    root_dir = r"/home/LinHonghui/Datasets/HW_ImageRetrieval/train_data_resize704",
                    images_per_classes=5,classes_per_minibatch=1),

        transforms = [
            # dict(type="RescalePad",output_size=320),
            # dict(type="DeGaussianNoise",p=0.1,loc=0,scale=(10,50)),
            # dict(type="DePepperNoise",p=0.1,percentage=0.1),
            dict(type="ShiftScaleRotate",p=0.3,shift_limit=0.1,scale_limit=(-0.4,0.3),rotate_limit=15),
            dict(type="IAAPerspective",p=0.1,scale=(0.05, 0.15)),
            dict(type="ChannelShuffle",p=0.1),
            dict(type="RandomRotate90",p=0.5),
            dict(type="RandomHorizontalFlip",p=0.5),
            dict(type="RandomVerticalFlip",p=0.5),
            dict(type="ColorJitter",brightness=0.1,contrast=0.1,saturation=0.1,hue=0.2),
    ...
    gallery_pipeline = dict(
        dataloader = dict(batch_size = 64,shuffle = False,num_workers = 16,drop_last = False,pin_memory=False,),
        dataset = dict(type="load_testB",
                    image_dir = r"/home/LinHonghui/Datasets/testC/testdata_1019/gallery/",
                    label_file = r"/home/LinHonghui/Datasets/testC/noise_type_gallery.txt"),
        transforms = [
            dict(type="de_MedianBlur",size=5),
            # dict(type="de_bilateralFilter",d=0, sigmaColor=100, sigmaSpace=12),
            dict(type="RescalePad",output_size=704),
            dict(type="ToTensor",),
    ...
    query_pipeline = dict(
        dataloader = dict(batch_size = 64,shuffle = False,num_workers = 16,drop_last = False,pin_memory=False,),
        dataset = dict(type="load_testB",
                    image_dir = r"/home/LinHonghui/Datasets/testC/testdata_1019/query/",
                    label_file = r"/home/LinHonghui/Datasets/testC/noise_type_query.txt"),
        transforms = [
            dict(type="de_MedianBlur",size=5),
            # dict(type="de_bilateralFilter",d=0, sigmaColor=100, sigmaSpace=12),
            dict(type="RescalePad",output_size=704),
    ...
    model = dict(
        net = dict(type="SBNet"),
        backbone = dict(type="efficient_b4"),
        aggregation = dict(type="GeneralizedMeanPoolingP",output_size=(1,1),),
        heads = dict(type="BNneckHead",in_feat=1792,num_classes=3097),
        losses = [
                # dict(type="AMLinear",in_features=1664,num_clssses=3097,m=0.35,s=30,weight=1/4),
                dict(type="ArcfaceLoss_Dropout",in_feat=1792,num_classes=3097,scale=35,margin=0.30,dropout_rate=0.2,weight=1),
    ...
    warm_up = dict(length=2000,min_lr=4e-6,max_lr=4e-4,froze_num_lyers=15)
    # Outpu
    # save_dir = r""
