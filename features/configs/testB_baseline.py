...
LastEditTime: 2020-09-27 07:52:13
'''
...
config = dict(
    ...
    #                           
    train_pipeline = dict(
        dataloader = dict(batch_size = 7,num_workers = 12,drop_last = True,pin_memory=True,
    ...
    gallery_pipeline = dict(
        dataloader = dict(batch_size = 120,shuffle = False,num_workers = 16,drop_last = False),
        dataset = dict(type="load_testB",
                    image_dir = r"/home/LinHonghui/Datasets/HW_ImageRetrieval/test_data_B/gallery/",
                    label_file = r"/home/LinHonghui/Datasets/HW_ImageRetrieval/test_data_B/noise_type_gallery.txt"),
        transforms = [
            dict(type="de_MedianBlur",size=5),
            dict(type="de_bilateralFilter",d=0, sigmaColor=100, sigmaSpace=12),
            dict(type="RescalePad",output_size=576),
            dict(type="ToTensor",),
    ...
    query_pipeline = dict(
        dataloader = dict(batch_size = 120,shuffle = False,num_workers = 16,drop_last = False),
        dataset = dict(type="load_testB",
                    image_dir = r"/home/LinHonghui/Datasets/HW_ImageRetrieval/test_data_B/query/",
                    label_file = r"/home/LinHonghui/Datasets/HW_ImageRetrieval/test_data_B/noise_type_query.txt"),
        transforms = [
            dict(type="de_MedianBlur",size=5),
            dict(type="de_bilateralFilter",d=0, sigmaColor=100, sigmaSpace=12),
            dict(type="RescalePad",output_size=576),
    ...
    optimizer = dict(type="Adam",lr=8e-4,weight_decay=1e-5),
