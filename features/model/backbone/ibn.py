...
def densenet121_ibn_a(pretrained=True, **kwargs):
...
def resnet101_ibn_a(pretrained=False, **kwargs):
    ...
    model = ResNet_IBN(block=Bottleneck_IBN,
                       ...
                       **kwargs)
    if pretrained:
        model.load_state_dict(torch.hub.load_state_dict_from_url(model_urls['resnet101_ibn_a'],map_location="cpu"))
    model = nn.Sequential(*list(model.children())[:-2])
    return model
...
if __name__ == "__main__":
    ...
    # model = densenet169_ibn_a(pretrained=True)
    # model = nn.Sequential(*list(model.children())[:-2])
