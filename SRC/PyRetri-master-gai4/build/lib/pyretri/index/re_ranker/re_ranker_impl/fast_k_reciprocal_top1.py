...
@RERANKERS.register
class Fast_KReciprocal_top1(ReRankerBase):
    ...
    def __init__(self, hps: Dict or None = None):
        super(Fast_KReciprocal_top1, self).__init__()
    ...
    def self_call(self, query_fea: torch.tensor, gallery_fea: torch.tensor,metric:MetricBase,  dis: torch.tensor or None = None,
    ...
    def re_sort(self,array_list,N_resort):
        fuse_array = []
        ...
        for i in tqdm(range(len_array)):
            ...
            for j in range(len_array_list):
                for k in range(1, N_resort+1):
                    key = array_list[j][i][k]
                    if key not in weight_dict:
                        weight_dict[key] = 1.0 / k
                    else:
                        weight_dict[key] = weight_dict[key] + 1.0 / k
        ...
        return np.array(fuse_array)


    def __call__(self, query_fea: torch.tensor, gallery_fea: torch.tensor,metric:MetricBase,  dis: torch.tensor or None = None,
                 sorted_index: torch.tensor or None = None) -> torch.tensor or np.ndarray:
        sorted_idx=self.self_call(query_fea, gallery_fea, metric, dis, sorted_index)
        query_fea_new=gallery_fea[sorted_idx[:,0]]
        sorted_idx_new=self.self_call(query_fea_new, gallery_fea, metric, dis, sorted_index)
        
        N_resort=10
        sorted_idx_2_N_resort=self.re_sort([sorted_idx,sorted_idx_new],N_resort)

        return sorted_idx_2_N_resort
