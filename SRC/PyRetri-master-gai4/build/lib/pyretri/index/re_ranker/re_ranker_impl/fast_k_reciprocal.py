...
@RERANKERS.register
class Fast_KReciprocal(ReRankerBase):
    ...
    def __init__(self, hps: Dict or None = None):
        super(Fast_KReciprocal, self).__init__(hps)
    ...
    def __call__(self, query_fea: torch.tensor, gallery_fea: torch.tensor,metric:MetricBase,  dis: torch.tensor or None = None,
        ...
        for i in tqdm(range(all_num)):
            ...
            for j in range(len(k_reciprocal_index)):
                ...
                #if len(np.intersect1d(candidate_k_reciprocal_index, k_reciprocal_index)) > 1. / 2 * len(
                #       candidate_k_reciprocal_index):
        ...
        sorted_idx = np.argsort(final_dist,axis=1)[:,:30]
