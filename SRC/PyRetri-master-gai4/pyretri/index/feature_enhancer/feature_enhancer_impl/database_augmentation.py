...
class DBA(EnhanceBase):
    ...
    def __init__(self, hps: Dict or None = None):
        ...
        # self.plda = PLDA(knn_hps)

    # def __call__(self, feature: torch.tensor) -> torch.tensor:
    def __call__(self, feature: torch.tensor,metric:MetricBase) -> torch.tensor:
        _, sorted_idx = self.knn(feature, feature)
        # jerry
        # self.metric=metric
        # self.metric._hyper_params["top_k"]=self._hyper_params["enhance_k"] + 1
        # _, sorted_idx = self.metric(feature, feature)
