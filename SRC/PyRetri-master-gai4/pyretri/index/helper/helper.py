...
class IndexHelper:
    ...
    def do_index(self, query_fea: np.ndarray, query_info: List, gallery_fea: np.ndarray) -> (List, np.ndarray, np.ndarray):
        ...
        gallery_fea = self.feature_enhance(gallery_fea,self.metric)
        print(self.metric._hyper_params["top_k"])
        ...
        # jerry
        sorted_index = self.re_rank(query_fea, gallery_fea,self.metric, dis=None, sorted_index=None)
        if len(sorted_index)==2:
            final_dist=sorted_index[1]
        ...
        return query_info, query_fea, gallery_fea,final_dist
