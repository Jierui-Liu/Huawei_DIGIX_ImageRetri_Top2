...
class RGA_Module(nn.Module):
    def __init__(self, in_channel, in_spatial, use_spatial=True, use_channel=True, \
        ...
        self.in_channel = in_channel  #t
        self.in_spatial = in_spatial  #长宽乘积
    ...
    def forward(self, x):
        ...
        if self.use_spatial:
            ...
            if not self.use_channel:
                out = torch.sigmoid(W_ys.expand(b,c,w,h)) * x
                return out
            else:
                # print(torch.sum(torch.abs(torch.sigmoid(tmp) * x-torch.sigmoid(W_ys) * x)))
                # x = torch.sigmoid(W_ys.expand_as(x)) * x
                x = torch.sigmoid(W_ys) * x


        if self.use_channel:
            # channel attention
            # xc = x.view(b, c, -1).permute(0, 2, 1).unsqueeze(-1)
            # xc = x.view(b, c, -1).permute(0, 2, 1).unsqueeze(3)
            # xc = x.flatten(2).permute(0, 2, 1).unsqueeze(3)
            xc = x.flatten(2).permute(0, 2, 1).view(batch,1024,channel,1)
            # xc = x.view(batch,1280,1024,1).transpose(2, 1)

            # theta_xc = self.theta_channel(xc).squeeze(-1).permute(0, 2, 1)
            # theta_xc = self.theta_channel(xc).squeeze(3).permute(0, 2, 1)
            theta_xc = self.theta_channel(xc).view(batch,128,channel).permute(0, 2, 1)

            # phi_xc = self.phi_channel(xc).squeeze(-1)
            # phi_xc = self.phi_channel(xc).squeeze(3)
            phi_xc = self.phi_channel(xc).view(batch,128,channel)
            Gc = torch.matmul(theta_xc, phi_xc)

            # Gc_in = Gc.permute(0, 2, 1).unsqueeze(-1)
            # print(Gc.permute(0, 2, 1).shape)
            # Gc_in = Gc.permute(0, 2, 1).unsqueeze(3)
            Gc_in = Gc.permute(0, 2, 1).view(batch,channel,channel,1)

            # Gc_out = Gc.unsqueeze(-1)
            # Gc_out = Gc.unsqueeze(3)
            # Gc_out = Gc.view(batch,1280,1280,1)
            Gc_out = Gc_in.transpose(1,2)
