...
def trans_json_to_here(here_dir='/home/LinHonghui/Datasets/Hw_ImageRetrieval/features/hw1',\
    ...
    for i in range(len(df["fname"])):
        ...
        for j in range(num_dir):
            if mode=='concat':
                dict_tmp['feature']['f{}'.format(j+1)]=(query_data_json[j][i,:]/np.linalg.norm(query_data_json[j][i,:])).tolist()
    ...
    for i in range(len(df["fname"])):
        ...
        for j in range(num_dir):
            if mode=='concat':
                dict_tmp['feature']['f{}'.format(j+1)]=(gallery_data_json[j][i,:]/np.linalg.norm(gallery_data_json[j][i,:])).tolist()
