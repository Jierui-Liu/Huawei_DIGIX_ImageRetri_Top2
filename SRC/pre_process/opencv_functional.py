...
LastEditTime: 2020-08-27 17:02:59
Description :
'''
...
def randomcrop(sample,output_size):
    ...
    if h<=new_h and w<=new_w: #存在图像尺寸小于crop_size的可能性
        image_new=np.zeros((new_h,new_w,3)).astype(np.uint8)
        image_new[:h,:w]=image
        sample['image'] = image_new
        # warnings.warn("image_size ({}) is smaller than expect output_size ({}).".format((w,h),image_new.shape))
        return sample
    elif h<=new_h:
        left = np.random.randint(0,w-new_w)
        image_new=np.zeros((new_h,new_w,3)).astype(np.uint8)
        image_new[:h,:]=image[:,left:left+new_w]
        sample['image'] = image_new
        # warnings.warn("image_size ({}) is smaller than expect output_size ({}).".format((w,h),image_new.shape))
        return sample
    elif w<=new_w:
        ...
        image_new=np.zeros((new_h,new_w,3)).astype(np.uint8)
        image_new[:,:w]=image[top:top+new_h,:]
        sample['image'] = image_new
        # warnings.warn("image_size ({}) is smaller than expect output_size ({}).".format((w,h),image_new.shape))
        return sample
...
def rescale_pad(sample,output_size,interpolation=cv.INTER_LINEAR,fill=0,padding_mode='constant'):
    image = sample['image']
    h,w = image.shape[:2]
    if h==output_size and w==output_size:  #没必要resize和PAD
        return sample
...
def rescale_pad_jr(sample,output_size):
    size_new=output_size[0]
    image = sample['image']
    h, w, c = image.shape
    if w > h:
...
def gaussiannoise_correction(sample, mean=0, sigma=25):
    image = sample['image']
    if not _is_numpy_image(image):
        raise TypeError("Image should be a numpu.ndarray image. Got {}".format(type(image)))

    h, w, c = image.shape
    image = image + np.random.normal(mean, sigma ,size=(h, w, c))
...
def peppernoise_recovery(sample,percentage=0.1):
    image = sample['image']
    if not _is_numpy_image(image):
        raise TypeError("Image should be a numpu.ndarray image. Got {}".format(type(image)))
...
def gaussiannoise_correction_recovery(sample,mean,sigma):
    image = sample['image']
    if not _is_numpy_image(image):
        raise TypeError("Image should be a numpu.ndarray image. Got {}".format(type(image)))
...
def gaussiannoise_bf_correction_recovery(sample,mean,sigma):
    image = sample['image']
    if not _is_numpy_image(image):
        raise TypeError("Image should be a numpu.ndarray image. Got {}".format(type(image)))
...
def gaussiannoise_ms_correction_recovery(sample,mean,sigma):
    image = sample['image']
    if not _is_numpy_image(image):
        raise TypeError("Image should be a numpu.ndarray image. Got {}".format(type(image)))
