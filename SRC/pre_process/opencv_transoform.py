...
class RescalePad_jr(object):
    def __init__(self,output_size):
        self.output_size = output_size
    def __call__(self,sample):
        sample = F.rescale_pad_jr(sample,self.output_size)
        return sample
...
class GaussianNoise_correction(object):
    def __init__(self,p=0.1,mean=0,sigma=25):
    ...
    def __call__(self,sample):
        if np.random.random() <= self.p:
            return F.gaussiannoise_correction(sample,self.mean,self.sigma)
        else:
            return sample
    def __repr__(self):
...
class PepperNoise_Recovery(object):
    def __init__(self,p=0.1,percentage=0.1):
        self.percentage = percentage
        self.p = p
    ...
    def __call__(self,sample):
        if np.random.random() <= self.p:
            return F.peppernoise_recovery(sample,self.percentage)
        else:
            return sample

class GaussNoise_Recovery(object):
    def __init__(self,p=0.1,mean=0,sigma=25):
        self.p = p
        self.mean=mean
        self.sigma=sigma
    ...
    def __call__(self,sample):
        if np.random.random() <= self.p:
            return F.gaussiannoise_correction_recovery(sample,self.mean,self.sigma)
        else:
            return sample


class GaussNoise_PepperNoise_Recovery(object):
    def __init__(self,p=0.1,percentage=0.1,mean=0,sigma=25):
        self.p = p
        self.mean=mean
        self.sigma=sigma
        self.percentage = percentage
    ...
    def __call__(self,sample):
        ...
        if random_s <= self.p/2:
            return F.gaussiannoise_correction_recovery(sample,self.mean,self.sigma)
        elif random_s <= self.p and random_s > self.p/2:
            return F.peppernoise_recovery(sample,self.percentage)
        else:
            return sample


class GaussNoise_BF_PepperNoise_Recovery(object):
    def __init__(self,p=0.1,percentage=0.1,mean=0,sigma=25):
        self.p = p
        self.mean=mean
        self.sigma=sigma
    ...
    def __call__(self,sample):
        random_s=np.random.random()
        if random_s <= self.p/2:
            return F.gaussiannoise_bf_correction_recovery(sample,self.mean,self.sigma)
        elif random_s <= self.p and random_s > self.p/2:
            return F.peppernoise_recovery(sample,self.percentage)
        else:
            return sample
...
class BF_Median(object):
    ...
    def __call__(self,sample):
        ...
        random_s=np.random.random()
        if random_s <= self.p/2:
            image = cv.bilateralFilter(src=image, d=0, sigmaColor=100, sigmaSpace=9)
        elif random_s <= self.p and random_s > self.p/2:
...
class GaussNoise_MS_PepperNoise_Recovery(object):
    def __init__(self,p=0.1,percentage=0.1,mean=0,sigma=25):
        self.p = p
        self.mean=mean
        self.sigma=sigma
    ...
    def __call__(self,sample):
        random_s=np.random.random()
        if random_s <= self.p/2:
            return F.gaussiannoise_ms_correction_recovery(sample,self.mean,self.sigma)
        elif random_s <= self.p and random_s > self.p/2:
            return F.peppernoise_recovery(sample,self.percentage)
        else:
            return sample
...
class GaussNoise_BF_PepperNoise_Recovery_Dynamic(object):
    def __init__(self,p=0.1,percentage=0.05,mean=0,sigma=20):
        self.p = p
        self.mean=mean
        self.sigma=sigma
        self.percentage = percentage
    ...
    def __call__(self,sample):
        random_s=np.random.random()
        if random_s <= self.p/2:
            return F.gaussiannoise_bf_correction_recovery(sample,self.mean,self.sigma+(np.random.random()-0.5)*10)
        elif random_s <= self.p and random_s > self.p/2:
            return F.peppernoise_recovery(sample,self.percentage+(np.random.random()-0.5)/15)
        else:
            return sample

       
class MeanBlur(object):
    def __init__(self,p=0.1,size=(5,5)):
        self.size = size
        self.p = p
    def __call__(self,sample):
        image = sample['image']
...
class MedianBlur(object):
    def __init__(self,p=0.1,size=5):
        self.size = size
        self.p = p
    def __call__(self,sample):
        image = sample['image']
...
class Roberts(object):
    ...
    def __call__(self,sample):
        ...
        image = cv.addWeighted(absX, 0.5, absY, 0.5, 0)
        image = np.clip(image, 0, 255)

        image = image[...,np.newaxis].astype(np.uint8)  # 默认np.uint8,to_tensor启动转换归一化
        sample['image'] = image
...
class Roberts_gamma(object):
    ...
    def __call__(self,sample):
        ...
        absX = cv.convertScaleAbs(x)
        absY = cv.convertScaleAbs(y)
        image = cv.addWeighted(absX, 0.5, absY, 0.5, 0)*1.0
        # image=(image-image.min())/((image.max()-image.min()))
        image=image/255.0
        image = image** (1/self.gamma)
        image *= 255
        image = np.clip(image, 0, 255)
...
class ImageCompression_channel(object):
    def __init__(self,p=0.07,quality_lower=1,quality_upper=2):
        self.p = p
        self.quality_lower = quality_lower
        self.quality_upper = quality_upper
        self.aug = A.ImageCompression(p=1,quality_lower=quality_lower,quality_upper=quality_upper)
    ...
    def __call__(self,sample):
        ...
        if np.random.random()<self.p:
            ...
            if rs<0.1:
                channel=2
                image[:,:,channel] =  self.aug(image=image[:,:,channel])["image"]
            elif rs<0.2:
                channel=0
                image[:,:,channel] =  self.aug(image=image[:,:,channel])["image"]
            elif rs<0.3:
