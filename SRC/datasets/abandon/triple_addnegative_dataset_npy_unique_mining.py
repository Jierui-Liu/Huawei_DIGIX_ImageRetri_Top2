...
class train_dataset(Dataset):
    def __init__(self,root_dir,images_per_classes,classes_per_minibatch,nums_addnegative,transform,cluster=2):
        ...
        for i in range(self.num_all_classes):
        ...
        while 1:
            ...
            if int(data_l[1]) not in self.label_parent_dict[int(data_l[2])]:
                self.label_parent_dict[int(data_l[2])].append(int(data_l[1]))
                class_id+=1
        ...
        for key in dict_tmp.keys():
            if len(dict_tmp)==0:
                ...
                self.label_parent_lst.append(self.label_parent_dict[key])
